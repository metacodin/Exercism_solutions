...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    generated_porwer = voltage*current
    efficiency = (generated_porwer/theoretical_max_power)*100
    if efficiency >= 80:
        return 'green'
    if 60 <= efficiency <80:
        return 'orange'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if temperature*neutrons_produced_per_second < 0.90*threshold:
        return 'LOW'
    if temperature*neutrons_produced_per_second >= 0.90*threshold and temperature*neutrons_produced_per_second <=1.10*threshold:
        return 'NORMAL'
    return 'DANGER'
